...
float4 PS_Main( PS_Input frag ) : SV_TARGET
{
    ...
    if( diffuseTerm > 0.0f )
    {
        ...
        float3 halfVec = normalize( lightToPixelVec + viewVec );
        specularTerm = pow( saturate( dot( frag.norm, halfVec ) ), specularPower );
    }
	
	finalAmbient *= diffuseTerm*lightIntensity1;
	finalAmbient += specularMaterColor*frag.LightDifuseColor *specularTerm;
	...
	finalColor += specularMaterColor*frag.LightDifuseColor *specularTerm;
