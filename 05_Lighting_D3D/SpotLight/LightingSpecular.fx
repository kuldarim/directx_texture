...
struct PS_Input
{
	...
	float difterm : TEXCOORD11;
	float speterm : TEXCOORD12;
};
...
PS_Input VS_Main( VS_Input vertex )
{
    ...
    float3 viewVec = normalize( cameraPos - vsOut.worldPos );

	float3 lightVec = normalize( LightPosition1 - vsOut.worldPos );
...
float4 PS_Main( PS_Input frag ) : SV_TARGET
{
	...
	finalColor = saturate(finalColor + finalAmbient + frag.LightAmbientColor + frag.speterm);
